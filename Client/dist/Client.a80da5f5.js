...
})({"9pAa2":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "46a9f20aa80da5f5";
"use strict";
...
const health = async function() {
    try {
        const res = await fetch(`${(0, _config.BASE_URL)}/health`);
        if (!res.ok) return {
            success: false
        };
...
class UserManageApi {
    ...
    async _apiCall(url, options) {
        try {
            let res;
            res = await fetch(`${(0, _config.BASE_URL)}${url}`, options);
    ...
    async _refreshAccessToken() {
        try {
            const res = await this._apiCall("/refresh", {
                method: "POST",
                credentials: "include"
            });
    ...
    async login(username, password) {
        try {
            const res = await this._apiCall("/login", {
                method: "POST",
                headers: {
    ...
    async getCurrentUser() {
        try {
            ...
            const res = await this._apiCall("/get", {
                headers: {
    ...
    async createUser({ email, ...others }) {
        try {
            ...
            const res = await this._apiCall("/create", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
            ...
            await this._calcRemainingDays(currentUser);
            await this._saveHowManyTimesClick(currentUser);
            console.log(data);
            return data;
        } catch (err) {
            console.error(err);
    ...
    async updateUser(updateInfo) {
        try {
            if (!this._accessToken) await this._refreshAccessToken();
            const res = await this._apiCall("/update/general", {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${this._accessToken}`
    ...
    async deleteUser(password) {
        try {
            if (!this._accessToken) await this._refreshAccessToken();
            const res = await this._apiCall("/delete", {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${this._accessToken}`
    ...
    async _calcRemainingDays(currentUser) {
        try {
            const remainingDays = currentUser.goals.map((goal)=>goal.date ? Math.ceil((new Date(goal.date) - new Date()) / 86400000) : undefined);
            const updatedUser = await this.updateUser({
                remainingDaysNow: remainingDays
            });
    ...
    async _saveHowManyTimesClick(currentUser) {
        try {
            const remainingDaysPrev = currentUser.remainingDaysPrev;
            const remainingDaysNow = currentUser.remainingDaysNow;
    ...
    async deleteAll() {
        try {
            ...
            console.error("Error while deleting Users and tokens", err);
        }
    }
    async _deleteAllUsers() {
        try {
            const res = await this._apiCall("/deleteAll", {
                method: "DELETE"
            });
            const data = await res.json();
            console.log(data);
    ...
    async _deleteAllTokens() {
        try {
            const res = await this._apiCall("/deleteTokens", {
                method: "DELETE"
            });
            const data = await res.json();
            console.log(data);
...
//   email: "erhsjk89@gmail.com",
// });
// console.log(updatedUser);
// const deleteUser = await auth.deleteUser("hdH73&jjH");
...
},{}]},["9pAa2","gHvsa"], "gHvsa", "parcelRequire3bdd", {})

//# sourceMappingURL=Client.a80da5f5.js.map
